...
/**
 * 基于NIO的TCP通信的Demo,客户端发送一串字符串到服务端,服务端将该字符串原原本本地反馈给客户端
 * @author gongchunru
 * @Package com.java.improve.NIODemo
 * @date 16/6/15 09:31
 */
public class TCPEchoClientNonblocking {
    public static void main(String[] args) throws Exception{
        ...
        //第一个参数作为要连接的服务端的主机名或者IP
        String server = args[0];
        //第二个参数为要发送到服务端的字符串
        byte[] argument = args[1].getBytes();
        //如果有第三方参数,则作为端口号,如果没有,则端口号为7
        ...
        if (!clntChan.connect(new InetSocketAddress(server,serverPort))){
            ...
            while (!clntChan.finishConnect()){
                //在等待连接的时间里,可以执行其他任务,以充分发挥飞阻塞IO的异步特性
                //这里为了掩饰该方法的使用,只是一直打印"."
                System.out.println(".");
            }
        }
        ...
        while (totalByteRcvd < argument.length){
            //如果用来向通道中写数据的缓冲区还有剩余的字节,则继续将数据写入信道.
